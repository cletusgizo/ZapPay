...
async function getWalletCredentials(): Promise<{
  address: string;
  privateKey: string;
}> {
  try {
    ...
    if (!address) {
      ...
      if (storedWallet) {
        ...
        console.warn(
          "No stored wallet data found for existing address, generating new wallet",
        );
        walletData = await generateStarknetAddress();
      }
    }
  ...
  try {
    ...
    if (amountInWei.includes("e") || amountInWei.includes("E")) {
      ...
      try {
        ...
        if (isNaN(exponent)) {
          throw new Error(
            `Invalid exponent in scientific notation: ${amountInWei}`,
          );
        }
        ...
        throw new Error(
          `Cannot convert scientific notation ${amountInWei} to regular format: ${e.message}`,
        );
      }
    }
    ...
    const amountBN = BigInt(formattedAmount);
    ...
    try {
      ...
      // If the error is related to scientific notation or BigInt, try a different approach
      if (sdkError.message && sdkError.message.includes("BigInt")) {
        console.log(
          "Retrying swap with reduced amount due to BigInt conversion error",
        );
        // Try with a much smaller amount (1 STRK in wei) using BigInt
        const smallerAmountBN = 10n ** 18n; // Correct 1 STRK in wei (10^18)
        console.log(`Retrying with smaller amount: ${smallerAmountBN}`);
        ...
        try {
          ...
          throw new Error(
            `Swap failed even with smaller amount: ${retryError.message}`,
          );
        }
      } else {
        throw sdkError;
...
export async function autoSwapStrkBalance(): Promise<{
  success: boolean;
  message: string;
  txHash?: string;
}> {
  try {
    ...
    console.log(
      ...
      strkBalanceStr.includes("e") || strkBalanceStr.includes("E"),
    );
    ...
    const amountToSwap =
      strkBalance > MAX_SWAP_AMOUNT ? MAX_SWAP_AMOUNT : strkBalance;
